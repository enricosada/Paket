    ...
    internal class ScriptWrapper
    {
        ...
        public void CreateWrapper(string target)
        {
            ...
            FileWriteAllLines(shellWrapperPath, new[] {
                ...
                "",
                "# have the paket script set this path to whatever relative directory",
                "rel_path='../paket-files/paket/bin'",

                "# anchor the path",
                "uname_str=$(uname)",
                @"if [[ ""$uname_str"" == 'Darwin' ]]; then",
                "absolute_path=$(realpath $rel_path)",
                @"elif [[ ""$uname_str"" == ""Linux"" ]]; then",
                "absolute_path=$(readlink -f $rel_path)",
                "fi",
                "",
                "# shamelessly taken from @mdl on the fpchat slack",
                "inpath()",
                "{",
                "    if [ $# -ne 1 ]",
                "    then",
                ...
                "}",
                "",
                "# see if the anchored path is in PATH",
                @"if (! inpath ""$absolute_path"" ); then",
                @"echo ""adding $rel_path to PATH""",
